...
from PyQt5.QtWidgets import QMainWindow, QMessageBox, QTableWidgetItem, QPushButton, QComboBox, QFileDialog
from pyqtlet import *
...
class HurturkGui(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui.cb_simulation.stateChanged.connect(self.is_simulation)
        ...
        self.ui.tw_waypoints.cellChanged.connect(self.waypoints_cell_changed)
        ...
        self.list_serial_port()

        self.ui.le_roll.textChanged.connect(self.changed_roll)
        self.ui.le_pitch.textChanged.connect(self.changed_pitch)

        self.autopilot = Autopilot(self.ui, self.markers, self.attitude_indicator_widget, self.planning_map, self.clear_map)
    ...
    def add_marker(self, point, mission="WAYPOINT", param1="", param2="", param3="", param4="", altitude=-1):
        marker_ok = True

        for marker_id, dict in self.markers.items():
            if geopy.distance.distance([point['latlng']['lat'], point['latlng']['lng']],
                                       [float(dict["enlem"]), float(dict["boylam"])]).m < float(self.ui.le_wpRadius.text()):
                marker_ok = False
        ...
        if marker_ok:
            ...
            self.markers[self.marker.layerId] = {"marker": self.marker,
                                                 ...
                                                 "irtifa": int(
                                                     self.ui.le_defaultAlt.text()) if altitude == -1 else altitude,
    ...
    def draw_all_markers(self, values=None):
        ...
        for tmp_marker in tmp_dict:
            ...
            self.add_marker(point=coord, mission=tmp_marker["komut"], altitude=tmp_marker["irtifa"])
    ...
    def complete_geofence(self):
        if len(self.geofence_markers) == 0:
            QMessageBox.warning(self, "Hata", "Geofence çizilmedi.")

        elif self.ui.le_minAlt.text() == "" or self.ui.le_maxAlt.text() == "":
            QMessageBox.warning(self, "Hata", "Minimum ve maksimum değer girilmedi.")
        elif int(self.ui.le_minAlt.text()) >= int(self.ui.le_maxAlt.text()):
            QMessageBox.warning(self, "Hata", "İrtifaları düzgün girin.")
        else:
    ...
    def waypoints_cell_changed(self, row, col):
        if self.ui.tw_waypoints.item(row, col).text() != "":
            if col == 2:
                self.markers[f"m_{row}"]["param1"] = int(self.ui.tw_waypoints.item(row, col).text())
            elif col == 3:
                self.markers[f"m_{row}"]["param2"] = int(self.ui.tw_waypoints.item(row, col).text())
            elif col == 4:
                self.markers[f"m_{row}"]["param3"] = int(self.ui.tw_waypoints.item(row, col).text())
            elif col == 5:
                self.markers[f"m_{row}"]["param4"] = int(self.ui.tw_waypoints.item(row, col).text())
            elif col == 8:
                self.markers[f"m_{row}"]["irtifa"] = int(self.ui.tw_waypoints.item(row, col).text())
    ...
    def selected_mission(self, new_mission):
        ...
        if new_mission == "LAND":
            list(self.markers.values())[int(mission_select[mission_select.rfind('_') + 1:])]["irtifa"] = 0
            self.ui.tw_waypoints.item(int(mission_select[mission_select.rfind('_') + 1:]), 8).setText(str(0))
    ...
    def create_table(self):
        ...
        self.ui.tw_waypoints.setColumnWidth(0, 30)
    ...
    def list_serial_port(self):
        serial_port_info = QSerialPortInfo()
        for serial_port in serial_port_info.availablePorts():
            self.ui.cb_connectionType.addItem(serial_port.portName())
    ...
    def mouseMoveEvent(self, event):
        ...
        self.window().move(self.window().x() + delta.x(),
                           self.window().y() + delta.y())  # başlangıç noktası kaydırılır
        self.oldPosition = event.globalPos()
